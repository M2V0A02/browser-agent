...
func (a *Agent) GetType() entity.AgentType {
	return entity.AgentTypeAnalysis
}
...
func (a *Agent) GetDescription() string {
	return "Analyze page content, take screenshots, verify information, and compare data. Use this when you need to examine, inspect, or validate what's on the current page."
}

func (a *Agent) Execute(ctx context.Context, task string) (string, error) {
	a.logger.Info("Analysis agent executing", "task", task)
	...
	for iter := 1; iter <= maxIterations; iter++ {
		a.logger.Debug("Analysis agent iteration", "iteration", iter)
...
func (a *Agent) filterTools() []entity.ToolDefinition {
	allowedTools := []string{"observe", "screenshot", "search", "scroll", "query_elements"}
